#!/usr/bin/env bash
# 03_filter.sh — VCF normalization + two-stage filtering
#
# Stage 1: bcftools norm   — left-align, split multiallelic
# Stage 2: soft-filter     — QUAL, DP (marked, not removed — VEP will see them)
# Stage 3: soft-filter AB  — allele balance for heterozygotes (FreeBayes: AO/RO)
#
# Result: ${RUN_DIR}/3_vcf_filtered/${SAMPLE_ID}.filtered.vcf.gz
#         (FILTER=PASS — passed all thresholds; LowQual/LowAB — marked)
set -euo pipefail
...
echo "[03] Normalization + filtration..."
bcftools norm \
    -f "$REF_FASTA" \
    -m -any \
    "$VCF_IN" \
...
echo "[03] Total: $TOTAL  |  PASS: $PASS"
echo "[03] Filtered VCF: $VCF_OUT"
