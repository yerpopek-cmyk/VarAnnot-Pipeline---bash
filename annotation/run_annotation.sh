...
# Functional annotation of a bacterial genome:
#   Prokka (structural) → DIAMOND blastp (homology) → hmmscan (Pfam domains)
#
# Usage:
#   bash run_annotation.sh [--offline] [--max-proteins N] [--cpus N]
#
# Options:
#   --offline          Skip downloading, use cache from data/db/
#   --max-proteins N   Limit the number of proteins for hmmscan (useful for testing)
#   --cpus N           Number of threads (default: nproc - 2, but at least 1)
# =============================================================================
set -euo pipefail
...
DIAMOND_DB="${DB_DIR}/swissprot_subset"   # without extension
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --cpus=*)            CPUS="${1#*=}";       shift ;;
        -h|--help)
            sed -n '/^# Usage/,/^# ===/p' "$0" | head -n 8
            exit 0 ;;
        *)  echo "❌ Unknown argument: $1"; exit 1 ;;
    esac
done

echo "⚙️  Threads: $CPUS / $TOTAL_CPUS  |  Output directory: $OUT_DIR"
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
    ...
    echo "   Activate the environment: conda activate annotation"
    exit 1
fi
...
if [[ "$OFFLINE" == false ]]; then
    ...
    # B. subtilis 168 Genome (~4 MB)
    if [[ ! -f "$ASSEMBLY" ]]; then
        echo "   B. subtilis genome..."
        ...
        echo "   Genome: using cache"
    fi

    # Swiss-Prot (first 100k records — ~40 MB instead of 600 MB)
    if [[ ! -f "$PROTEIN_DB_FASTA" ]]; then
        echo "   Swiss-Prot (subset)..."
        wget -qc --show-progress \
        ...
        echo "   Swiss-Prot: using cache"
    fi

    # Pfam — only 10 "training" domains (ABC transporters, chaperones, etc.)
    if [[ ! -f "${HMM_DB}.h3i" ]]; then
        echo "   Pfam-A (domain subset)..."
        wget -qc --show-progress \
...
' "$DOMAINS" "${DB_DIR}/Pfam-A.hmm" > "$HMM_DB"
        ...
        echo "   Pfam: using cache"
    fi
else
    echo -e "\n⚠️  Offline mode: skipping download"
    for f in "$ASSEMBLY" "$PROTEIN_DB_FASTA" "${HMM_DB}.h3i"; do
        [[ -f "$f" ]] || { echo "❌ Cache not found: $f"; exit 1; }
    done
fi
...
if [[ ! -f "${DIAMOND_DB}.dmnd" ]]; then
    ...
    echo -e "\n🔨 DIAMOND DB: using cache"
fi

# =============================================================================
# 4. Prokka — structural annotation
# =============================================================================
echo -e "\n🧬 Prokka..."
prokka \
...
# =============================================================================
# 5. DIAMOND blastp — functional homology
# =============================================================================
echo -e "\n🔎 DIAMOND blastp..."
diamond blastp \
    --db "$DIAMOND_DB" \
